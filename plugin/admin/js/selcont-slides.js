jQuery(document).ready(function($){
	...
	jQuery( '.add_project_images' ).on( 'click', 'a', function( event ) {
		...
		project_gallery_frame = wp.media.frames.downloadable_file = wp.media({
			...
			title: selcont_admin.gallery_title,
			button: {
				text: selcont_admin.gallery_button
			},
			multiple: true
		});
					...
					$project_images.append('\
						<li class="image" data-attachment_id="' + attachment.id + '">\
							<img src="' + attachment.sizes.thumbnail.url + '" />\
								<ul class="actions">\
									<li><a href="#" class="delete" title="'+ selcont_admin.delete_image +'">&times;</a></li>\
								</ul>\
							</li>');
					}
	...
	jQuery( '#project-data' ).on( 'click', '.projects-upload', function( event ) {
		...
		title = $button.data( 'title' ) ? $button.data( 'title' ) : selcont_admin.default_title;
		button = $button.data( 'button' ) ? $button.data( 'button' ) : selcont_admin.default_button;
